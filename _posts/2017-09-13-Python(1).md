...
变量名必须是`大小写英文`、`数字`和`_`的组合，且不能用数字开头

#### 一、整数

Python 可以处理任意大小的整数，当然包括负整数。计算机由于使用二进制，所以，有时候用十六进制表示整数比较方便，十六进制用`0x`前缀和`0-9`，`a-f`表示，例如：`0xff00`，`0xa5b4c3d2`，等等。
