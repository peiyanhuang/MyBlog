---
layout: post
title:  Vue 源码学习-数据响应系统
date:   2018-10-28 19:58:00 +0800
categories: Vue
tag: Vue
---
...
上面这段代码的意思是在非生产环境下如果发现在 `methods` 对象上定义了同样的 `key`，也就是说 `data` 数据的 `key` 与 `methods` 对象中定义的函数名称相同，那么会打印一个警告，提示开发者：**你定义在 `methods` 对象中的函数名称已经被作为 `data` 对象中某个数据字段的 `key` 了，你应该换一个函数名字**。
...
为什么当被读取的属性是数组的时候需要调用 `dependArray` 函数？
...
该 `if` 语句块的判断条件中包含两个函数，分别是 `isUndef` 和 `isPrimitive`。`isUndef` 函数用来判断一个值是否是 `undefined` 或 `null`，如果是则返回 `true`，`isPrimitive` 函数用来判断一个值是否是原始类型值，如果是则返回 `true`。所以如上代码 `if` 语句块的作用是：**如果 `set` 函数的第一个参数是 `undefined` 或 `null` 或者是原始类型值，那么在非生产环境下会打印警告信息**。这么做是合理的，因为理论上只能为对象(或数组)添加属性(或元素)。
...
如果代码运行到了这里，那说明正在给对象添加一个全新的属性，先定义了 `ob` 常量，它是数据对象 `__ob__` 属性的引用。然后使用 `defineReactive` 函数设置属性值，这是为了保证新添加的属性是响应式的。之后调用了 `__ob__.dep.notify()` 从而触发响应。这就是添加全新属性触发响应的原理。
...
[揭开数据响应系统的面纱](https://github.com/HcySunYang/vue-design/blob/master/docs/art/7vue-reactive.md)